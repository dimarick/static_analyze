 ...
 */ //Ошибка лучше изменить структуру цикла из-за инкремента
 ...
 *	       //Ошибка импортировал компаратор
 * @see Comparator
 * @exception IndexOutOfBoundsException index is out of range (<tt>size()&gt;0 && (from&lt;0 || from&gt;to || to&gt;=size())</tt>).
 */
 ...
 */ //Ошибка небезопасное приведение типов, пришлось изменить структуру
public void replaceFromWith(int from, java.util.Collection other) {
    checkRange(from, size());
    java.util.Iterator<Byte> e = other.iterator();
    int index = from;
...
public java.util.ArrayList<Byte> toList() {
    ...
    for (int i = 0; i < mySize; i++) {
        list.add(get(i)); // autoboxing автоматически использует Byte.valueOf()
    }
    return list;
}
/**
